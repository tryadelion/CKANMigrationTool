...
namespace MigrationToolPlugin
{
    public partial class MigrationToolUI : Form
    {
        ...
        void MigrateMods(object sender, DoWorkEventArgs e)
        {
            ...
            var mods = Main.Instance.mainModList.Modules;
        ...
        List<string> GetAutodetectedMods()
        {
            ...
            foreach (var mod in Main.Instance.mainModList.Modules)
            {
                try
                {
                    if (mod.IsAutodetected)
                    {
                        autodetected.Add(mod.Identifier);
                    }
                }
                catch (Exception ex)
                {
                    log.WarnFormat("Failed to get LatestAvailable() for {0} - {1}", mod, ex.Message);
                }
            }
            return autodetected;
        }
